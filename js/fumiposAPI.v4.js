(function (root, factory) {
  ...
  if (typeof require === "undefined" && typeof _Crystal === "object") {
    Crystal = _Crystal;
  } else {
  ...
  fumiposAPI.createIndexList = function (_fumipo) {
    if (!_fumipo.plotData[0][0]) return false;

    let keys = Object.keys(_fumipo.plotData[0][0].value);
    ...
    (function (menu) {
      fumiposAPI.loadHTML(menu.dataFile)
        .then(function (_r) {
          ...
          document.getElementById("use_test_data").addEventListener('click', function (ev) {
            let url = "./data/lava_compositions.csv";
            load(url).then(function (response) {
              return response.text();
            }).then(function (text) {
              let df = tf.text2Object(text, 'csv');
              df.map((v) => {
                if (!v.value.dummy) v.value.dummy = 1;
              });
              fumipo.plotData.push(df);
              d3.select("#selectedMainFile").text(url);
              fumiposAPI.createIndexList(fumipo);
            })
          }, false);

          document.getElementById('use_test_ref').addEventListener('click', function (ev) {
          ...
          document.getElementById("selectFileMain").addEventListener('change', function (ev) {
            ...
            reader.onload = function (ev) {
              let df = tf.text2Object(reader.result, 'csv');

              df.map((v) => {
                if (!v.value.dummy) v.value.dummy = 1;
              });

              fumipo.plotDatas.push(df);
          ...
          document.getElementById("selectFileRef").addEventListener('change', function (ev) {
            var file = ev.target.files;
            var reader = new FileReader();
      ...
      fumiposAPI.loadHTML(menu.simulate)
        .then(function (_r) {
          ...
          document.getElementById("selectMeltingFile").onchange = function (evt) {
            ...
            reader.onload = function (ev) {
              fumipo.partitioningData = text2Object(reader.result, 'csv');
  ...
  /* get extent of data 
    Arrayで渡されるデータセットリスト全体の最大最小値を求める
  */
  fumiposAPI.Binary.prototype.getDataExtent = function (dataLists) {
    let dataList = dataLists[0];
    ...
    for (let data of dataList) {
      ...
      for (let m of ["max", "min"]) {
        if (isNaN(x[m])) {
          w.x[m].push(d3[m](dataList, function (d) { return +d.value[x.sup] / +d.value[x.sub]; }));
        } else {
          w.x[m].push = parseFloat(x[m]);
        }

        if (isNaN(y[m])) {
          w.y[m].push(d3[m](dataList, function (d) { return +d.value[y.sup] / +d.value[y.sub]; }));
        } else {
          w.y[m].push = parseFloat(y[m]);
        }

      }
    }

    extent.x[0] = Math.min(...w.x.min);
    extent.x[1] = Math.max(...w.x.max);
    extent.y[0] = Math.min(...w.y.min);
    extent.y[1] = Math.max(...w.y.max);
  ...
  /* Replot flow */
  fumiposAPI.Binary.prototype.replot = function (dfPlots, csvRefObj, formDOM, dfPartitioning) {
    this.formObj = this.readForm(formDOM);
    var extent = this.getDataExtent(dfPlots);
    this.updateFramework(extent);
    dfPlots.map((df, layerNum) => this.plotPoint(df, extent, dfPartitioning, layerNum));
  }

  /* Main data plot */
  fumiposAPI.Binary.prototype.plotPoint = function (dataObj, extent, csvDObj, layerNum) {
    /* インスタンスの変数 */
    ...
    /* d3によるcircle要素の定義と登録 */
    var circle = svg.DOM.selectAll("circle.L" + layerNum).data(dataObj);
    ...
    circle.each(function (d) {
      ...
      d3.select(this).attr("r", function (d) {
        ...
        .attr("class", function (d) { return ("Binary L" + layerNum + "D" + d.value["ID"] + " " + d.value[styleClass] + " " + d.value["study"]) })
  ...
  /* Replot flow */
  fumiposAPI.Abundance.prototype.replot = function (dfPlots, csvRefObj, formDOM, dfPartitioning) {
    this.catchFormUpdate(formDOM);
    this.setNormList(csvRefObj);
    var layerNum = 0;
    var dataList = this.getNormalizedList(dfPlots);
    this.updateFramework();
    this.plotLine(dataList, layerNum);
  };

  /* plot main data */
  fumiposAPI.Abundance.prototype.plotLine = function (dataList, layerNum) {
    ...
    /**/
    path = svg.DOM.selectAll("path.abundance.L" + layerNum).data(dataList);
    path.exit().remove();
    path.enter().append("path");
    path.attr("name", function (d) { return d.value["name"]; })
      .attr("fill", "none")
      .attr("class", function (d) { return "abundance L" + layerNum + "D" + d.value["ID"] + " " + d.value[styleClass] + " " + d.value["study"] })
      .attr("d", function (d) { return line(getLineObj(d.norm)); })
      .attr("stroke-width", baseWidth);

    dataList.map((r, i) => {
      let circle = svg.DOM.selectAll("circle.path.L" + layerNum + "D" + r.value.ID).data(elementList);
      circle.exit().remove();
      circle.enter().append("circle");
      circle.attr("cx", function (d) { return axis.xScale(d) + offset.x + padding.left; })
        ...
        .attr("class", function (d) { return "path L" + layerNum + "D" + r.value.ID + " " + r.value[styleClass] })
        .attr("r", 2);
    })
