...
//import * as Crystal from "../multiCrystallization/js/multiCrystallization_v1.4.1.js";
//import * as Matrix from "../jslib/matrix.js";

fumiposAPI.Crystal = Crystal.setup(Matrix);
...
fumiposAPI.workSpaceInitialize=function(fumipo){
	...
	if (setting.component.fixedMenu){
		...
		/* ファイル読み込み */
			...
			/* Button for use deault data*/
		document.getElementById("use_test_data").onclick=function(ev){
			var data = new XMLHttpRequest();
			var url=setting.userDataSetting.url || "./data/lava_compositions.csv"	;
			data.open("GET",url,false);
			data.send(null);
			fumipo.plotData=text2Object(data.responseText,'csv');
		...
		document.getElementById('use_test_ref').onclick=function(ev){
			...
			url=setting.referranceDataSetting.url || "./data/Refferencial_abundance.csv";
			refData.open("GET",url,false);
			refData.send(null);
			fumipo.refData=text2Object(refData.responseText,'csv');
		...
		document.getElementById("selectFileMain").onchange=function(ev){
			...
			reader.onload=function(ev){
				fumipo.plotData=text2Object(reader.result,'csv');
		...
		document.getElementById("selectFileRef").onchange=function(ev){
			...
			//console.log(file)
			reader.onload=function(ev){
				fumipo.refData=text2Object(reader.result,'csv');
	...
	// グラフの共通スタイルの読みこみ
	if (setting.legend.commonStyleURL){
		var url=setting.legend.commonStyleURL;
	}else{
		url='./css/graph_common.css';
	}
	(function(url){
		/* グラフの共通css読み込み */
		var _url=url;
		$.ajax({
			...
			url:_url,
			success:function(data){
				$('#graph_style').append(data).trigger("create");
			},
			error: function(){
				alert("Failed to load css: "+_url);
			}
		});
	})(url)
	...
	/* form にマウスイベントを設定 */
		...
		document.getElementById("hideAll_button")
			.addEventListener('click',function(){fumiposAPI.allHide(),false});
		document.getElementById("showAll_button")
			.addEventListener('click',function(){fumiposAPI.allShow()},false);
	
		/* JSON を読みこんだら表に反映 */
	
		document.getElementById("legendJSON").onchange=function(ev){
			var file=ev.target.files;
			var reader=new FileReader();
			reader.readAsText(file[0]);
			//console.log(file)
...
fumiposAPI.setLegendFromUrl=function(url){
		...
		$.ajax({
			...
			success: function(res){
				fumiposAPI.createLegendTable(res);
			},
			error:function(){alert("error in reading legend file"+_url)},
		});
		d3.select('#legendFileLabel').text(_url);
};
...
fumiposAPI.createLegendTable=function(legendObj){
	...
	style+='<style type="text/css" id="legendStyle">'
		for (key in legendObj.legends){
			...
			if (obj.class){
				if (! obj.stroke){
					style+='circle.Binary.'+obj.class+'{fill:'+obj.color+';}';
				}else{
					style+='circle.Binary.'+obj.class+'{fill:#ffffff;stroke:'+obj.color+';}';
				};
				style+='path.abundance.'+obj.class+'{stroke:'+obj.color+';}';style+='td.'+obj.class+'{color:'+obj.color+';}';
			}else{
				if (! obj.stroke){
					style+='circle.Binary{fill:'+obj.color+';}';
				}else{
					style+='circle.Binary{fill:#ffffff;stroke:'+obj.color+';}';
				};
...
fumiposAPI.toggleVisibility=function(){
	...
	legendSimbol.each(function(d){
		...
		if(visiblityState == "hidden"){
			d3.selectAll("circle.Binary."+classArray[1])
...
fumiposAPI.GraphAppender.prototype.setGraph=function(graphType,formSetting){
	...
	fumipo.formDOM[this.graphNumCounter]=$('#form'+graphTypeString+graphNum+' > form');

	fumipo.plotter[this.graphNumCounter]=new Constructor(`div.graph${graphTypeString}.${graphTypeString+graphNum}`,fumipo.formDOM[this.graphNumCounter][0],fumipo.plotter);

	//console.log(formDOM[this.graphNumCounter][0])
		/* Add eventListenor*/
	d3.select("div#form"+graphTypeString+graphNum).select("form")
			.attr("onchange",`fumipo.plotter[${this.graphNumCounter}].replot(fumipo.plotData,fumipo.refData,fumipo.formDOM[${this.graphNumCounter}][0],fumipo.partitioningData);fumiposAPI.toggleVisibility()`);
	
	fumipo.plotter[this.graphNumCounter].replot(fumipo.plotData,fumipo.refData,fumipo.formDOM[this.graphNumCounter][0],fumipo.partitioningData);
	fumiposAPI.toggleVisibility()
...
fumiposAPI.Binary.prototype.readForm=function(formDOM){
	...
	/* Filtering data */
fumiposAPI.Binary.prototype.removeInvalidData=function(csvObj){
...
fumiposAPI.Binary.prototype.replot=function(csvObj,csvRefObj,formDOM,csvDObj){
	...
	this.plotPoint(dataObj,extent,csvDObj);
}
...
fumiposAPI.Binary.prototype.plotSimulatedMelt=function(endMember,_csvDObj){
	...
	function getRayleighCrystallizationMajor(endMember,x,y,phaseObj,D){
		...
		var crystal=new fumiposAPI.Crystal(initialMelt,phaseObj,elements);
